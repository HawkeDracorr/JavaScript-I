...
inventory.forEach(function(carObject) {
    carModels.push(carObject.car_model);
});
...
inventory.forEach(function(carObject) {
  carYears.push(carObject.car_year);
});
...
inventory.forEach (function(carObject) {
  if (carObject.car_year < 2000) {
    oldCars.push(carObject.car_year);
  };
});
...
inventory.forEach(function(carObject) {
  if (carObject.car_make === "BMW") {
    BMWAndAudi.push(carObject);
  } else if (carObject.car_make === "Audi") {
    BMWAndAudi.push(carObject);

  }
});

let GermanCars = JSON.stringify(BMWAndAudi);
